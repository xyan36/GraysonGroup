...
R = np.sqrt(X**2 + Y**2)
theta = np.arctan(Y/ X)
theta.iloc[20:30] = np.pi * 2 - theta.iloc[20:30]
...
axs[0].scatter(graph_data.Lockin1f, R * np.cos(theta - 90), label = 'X2')
axs[0].scatter(graph_data.Lockin2f, R * np.sin(theta - 90), label = 'Y2')
